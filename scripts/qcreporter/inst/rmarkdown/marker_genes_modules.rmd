...
for (sample in samples) {
  scrna.list[[sample]][["percent.mito"]] <- PercentageFeatureSet(scrna.list[[sample]], pattern = "^mt:") 
...
ggarrange(prefilter_qc_mito,postfilter_qc_mito)
```
...
ggarrange(prefilter_qc_ribo,postfilter_qc_ribo)
```
...
ggarrange(UMIs_transcripts_per_cell_prefilter,UMIs_transcripts_per_cell_postfilter)
```
...
ggarrange(genes_per_cell_pre,genes_per_cell_post)
```
<a  id="Complexity"></a> 
### Complexity
```{r Complexity}
metadata_prefilter$log10GenesPerUMI <- log10(metadata_prefilter$nFeature_RNA) / log10(metadata_prefilter$nCount_RNA)
metadata_postfilter$log10GenesPerUMI <- log10(metadata_postfilter$nFeature_RNA) / log10(metadata_postfilter$nCount_RNA)
# Visualize the overall complexity of the gene expression by visualizing the genes detected per UMI (novelty score)
genes_detected_per_umi_pre <- metadata_prefilter %>%
  	ggplot(aes(x=log10GenesPerUMI, color = orig.ident, fill=orig.ident)) +
  	geom_density(alpha = 0.2) +
  	theme_classic() +
  	geom_vline(xintercept = 0.8) + 
    ggtitle("Genes Detected per Umi Prefilter")

genes_detected_per_umi_post <- metadata_postfilter %>%
  	ggplot(aes(x=log10GenesPerUMI, color = orig.ident, fill=orig.ident)) +
  	geom_density(alpha = 0.2) +
  	theme_classic() +
  	geom_vline(xintercept = 0.8) + 
    ggtitle("Genes Detected per Umi Postfilter")

ggarrange(genes_detected_per_umi_pre,genes_detected_per_umi_post)
```
<a  id="Mito Gene Expression Per Cell"></a> 
### Mito Gene Expression Per Cell
```{r Mito Gene Expression Per Cell}
# Visualize the distribution of mitochondrial gene expression detected per cell
mitochondrial_gene_expression_per_cell_pre <- metadata_prefilter %>% 
  	ggplot(aes(color=orig.ident, x=percent.mito, fill=orig.ident)) + 
  	geom_density(alpha = 0.2) + 
  	scale_x_log10() + 
  	theme_classic() +
  	geom_vline(xintercept = 0.2) + 
    ggtitle("Mitochondrial Gene Expression Detected per Cell Prefilter")

mitochondrial_gene_expression_per_cell_post <- metadata_postfilter %>% 
  	ggplot(aes(color=orig.ident, x=percent.mito, fill=orig.ident)) + 
  	geom_density(alpha = 0.2) + 
  	scale_x_log10() + 
  	theme_classic() +
  	geom_vline(xintercept = 0.2) + 
    ggtitle("Mitochondrial Gene Expression Detected per Cell Postfilter")

ggarrange(mitochondrial_gene_expression_per_cell_pre,mitochondrial_gene_expression_per_cell_post)
```
