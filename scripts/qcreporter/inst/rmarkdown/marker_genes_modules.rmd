...
```{r QC}
# convert counts geneID to geneSymbol
counts_list <- list()
for (sample in samples){
    counts_list[[sample]] <- GetAssayData(scrna.list[[sample]],assay="RNA",slot="counts")
}                                    
gene_id_list = list()
labels = list()
new_gene_list = list()
for (sample in samples){
    gene_id_list[[sample]] = row.names(counts_list[[sample]])
    labels[[sample]] = geneTable$geneSymbol[match(gene_id_list[[sample]],geneTable$geneID)]
    labels[[sample]] = labels[[sample]] %>% replace_with_na_all(condition = ~.x == "")
    labels[[sample]] = as.character(labels[[sample]])
    names(labels[[sample]]) = gene_id_list[[sample]][!is.na(labels[[sample]])]
    new_gene_list[[sample]] = ifelse(is.na(labels[[sample]][gene_id_list[[sample]]]),gene_id_list[[sample]],labels[[sample]])
    new_gene_list[[sample]] = unname(new_gene_list[[sample]])
    row.names(counts_list[[sample]]) <- new_gene_list[[sample]]
}
# create new Seurat Object with geneSymbols as the row names                                        
so_prefilter = list()
for (sample in samples){
    so_prefilter[[sample]] <- CreateSeuratObject(counts=counts_list[[sample]],min.cells=3,project=sample)
}
  
# add sample name
for(i in 1:length(samples)){
  sample=samples[i]; treatment=treatments[i];
  so_prefilter[[sample]]$treatment <- treatment
}

  
# create new merged seurat object
merged_so_prefilter <- Merge_Seurat_List(so_prefilter,
                               add.cell.ids = NULL,
                               merge.data = TRUE,
                               project = "SeuratProject")      
# Add percent.mito and percent.ribo to cell level metadata
# manually calculate 
# mito
mt.genes <- rownames(merged_so_prefilter)[grep("^MT:|MT-",rownames(merged_so_prefilter))]
C<-GetAssayData(object = merged_so_prefilter, slot = "counts")
percent.mito <- colSums(C[mt.genes,])/Matrix::colSums(C)*100
merged_so_prefilter <- AddMetaData(merged_so_prefilter, percent.mito, col.name = "percent.mito")
# ribo
rb.genes <- rownames(merged_so_prefilter)[grep("^RP[SL]",rownames(merged_so_prefilter))]
percent.ribo <- colSums(C[rb.genes,])/Matrix::colSums(C)*100
merged_so_prefilter <- AddMetaData(merged_so_prefilter, percent.ribo, col.name = "percent.ribo")
  
# grab prefilter meta
pre_filter_metadata <- merged_so_prefilter@meta.data
...
ribo_cutoff = 10
for (sample in samples){
  mean.nCount <- mean(merged_so_prefilter@meta.data$nCount_RNA)
  sd.nCount <- sd(merged_so_prefilter@meta.data$nCount_RNA)
  mean.nFeature <- mean(merged_so_prefilter@meta.data$nFeature_RNA)
  sd.nFeature <- sd(merged_so_prefilter@meta.data$nFeature_RNA)
  so_postfilter <- subset(merged_so_prefilter, subset = nCount_RNA > mean.nCount - qc_cutoff*sd.nCount & nCount_RNA < mean.nCount + qc_cutoff*sd.nCount & nFeature_RNA > mean.nFeature - qc_cutoff*sd.nFeature & nFeature_RNA < mean.nFeature + qc_cutoff*sd.nFeature & percent.mito < mito_cutoff & percent.ribo < ribo_cutoff)
}
...
# QC Plots
# vln plot mito 
pdf(paste0(out_dir, "vlnplot_qc_mito_filtered.pdf"))
VlnPlot(so_postfilter, features = "percent.mito", pt.size = 0.1) + NoLegend()
dev.off()

# vln plot ribo 
pdf(paste0(out_dir, "vlnplot_qc_ribo_filtered.pdf"))
VlnPlot(so_postfilter, features = "percent.ribo", pt.size = 0.1) + NoLegend()
dev.off()

# scatter mito
pdf(paste0(out_dir, "scatter_qc_mito_filtered.pdf"))
FeatureScatter(so_postfilter, feature1 = "nFeature_RNA", feature2 = "percent.mito")
dev.off()

# scatter ribo
pdf(paste0(out_dir, "scatter_qc_ribo_filtered.pdf"))
FeatureScatter(so_postfilter, feature1 = "percent.ribo", feature2 = "nFeature_RNA")
dev.off()

pdf(paste0(out_dir, "scatter_qc_count_feature_filtered.pdf"))
FeatureScatter(so_postfilter, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
dev.off()
...
```{r rna_Elbow_Plot}
p1 <- ElbowPlot(so_postfilter) + ggtitle("Elbow Plot") + theme(aspect.ratio=5/10) + theme(plot.margin = unit(c(3, 3, 3, 3), "cm"))
...
res_1.4
dev.off()
Idents(so_postfilter) <- "RNA_snn_res.0.5"
